...
namespace engine {
	...
	glm::mat4 Camera::orthoProjection(glm::vec2 viewport)
	{
		viewport = glm::normalize(viewport);
		float width = size.x * viewport.x; /*std::cout << "Width: " << width << std::endl;*/
		float height = size.y * viewport.y;
